...
class UpSampleBN(nn.Module):
    ...
    def forward(self, x, concat_with):
        dtype = x.dtype
        up_x = F.interpolate(x.to(torch.float32), size=[concat_with.size(2), concat_with.size(3)], mode='bilinear', align_corners=True).to(dtype)
        f = torch.cat([up_x, concat_with], dim=1)
        return self._net(f)

class DecoderBN(nn.Module):
    def __init__(self, num_features=2048, num_classes=1, bottleneck_features=2048, basemodel_name='tf_efficientnet_b5_ap'):
        ...
        if self.basemodel_name == 'tf_efficientnet_b5_ap':
    ...
    def forward(self, features):
        ...
        # print(x_block0.shape, x_block1.shape, x_block2.shape, x_block3.shape, x_block4.shape, sep='\n')
...
class UnetAdaptiveBins(nn.Module):
    def __init__(self, backend, n_bins=100, min_val=0.1, max_val=10, norm='linear', basemodel_name='tf_efficientnet_b5_ap'):
        ...
        if self.basemodel_name == 'tf_efficientnet_b5_ap':
            self.decoder = DecoderBN(num_classes=128, basemodel_name=self.basemodel_name)
        else:
            self.decoder = DecoderBN(num_classes=128, basemodel_name=self.basemodel_name, bottleneck_features=1280)
    ...
    def forward(self, x, **kwargs):
        ...
        return {
            ...
            'bin_prob': out,
            'bin_edges': bin_edges,
            'centers_normed': (centers - self.min_val) / (self.max_val - self.min_val),
            'pred': pred
        }
    ...
    @classmethod
    def build(cls, n_bins, basemodel_name = 'tf_efficientnet_b5_ap', **kwargs):
        ...
        '''
        print('Quantinizing base model...', end='')
        backend = "fbgemm"
        basemodel.qconfig = torch.quantization.get_default_qconfig(backend)
        torch.backends.quantized.engine = backend
        basemodel = torch.quantization.prepare(basemodel, inplace=False)
        basemodel = torch.quantization.convert(basemodel, inplace=False)
        print('Done.')'''

        # Building Encoder-Decoder model
        print('Building Encoder-Decoder model..', end='')
        m = cls(basemodel, n_bins=n_bins, basemodel_name=basemodel_name, **kwargs)
        print('Done.')
        return m
