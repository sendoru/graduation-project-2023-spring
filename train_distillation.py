...
def distillation_loss(output_dict, teacher_output_dict, depth, criterion_ueff, criterion_bins, args):

    
    loss = torch.Tensor(0.).to(output_dict['pred'].device)
    ...
    if args.w_chamfer > 0:
        ...
        l_chamfer = torch.Tensor([0]).to(pred.device)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.UnetAdaptiveBins.build(n_bins=args.n_bins, min_val=args.min_depth, max_val=args.max_depth,
                                          norm=args.norm, basemodel_name=args.encoder_backend)
    teacher_model = models.UnetAdaptiveBins.build(n_bins=args.n_bins, min_val=args.min_depth, max_val=args.max_depth,
                                          norm=args.norm)
    ...
    teacher_model, _, _= model_io.load_checkpoint(args.teacher_model, teacher_model)
    ...
    train(model, teacher_model, args, epochs=args.epochs, lr=args.lr, device=args.gpu, root=args.root,
          experiment_name=args.name, optimizer_state_dict=opt)


def train(model, teacher_model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    criterion_prob = ProbDifferenceLoss()
    criterion_center = CenterDifferenceLoss()
    criterion_feature = FeatureMapDifferenceLoss()
    ################################################################################################
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            student_output_dict = model(img)
            if args.w_bound > 0 or args.w_center > 0 or args.w_feature > 0:
                teacher_output_dict = teacher_model(img)

            bin_edges, pred = student_output_dict['bin_edges'], student_output_dict['pred']
            if args.quant != '':
                bin_edges = bin_edges.float()
                pred = pred.float()
                for k, v in student_output_dict.items():
                    student_output_dict[k] = v.float()
                if args.w_bound > 0 or args.w_center > 0 or args.w_feature > 0:
                    for k, v in teacher_output_dict.items():
                        teacher_output_dict[k] = v.float()
            ...
            if args.w_bound > 0:
                l_boundary = criterion_boundary(pred, teacher_output_dict['pred'], depth, 0.01, mask)
            else:
                l_boundary = 0
            if args.w_center > 0:
                l_center = criterion_center(student_output_dict['centers_normed'], teacher_output_dict['centers_normed'])
            else:
                l_center = 0
            if args.w_prob > 0:
                l_prob = criterion_prob(student_output_dict['bin_prob'], teacher_output_dict['bin_prob'])
            else:
                l_prob = 0
            if args.w_feature > 0:
                l_feature = criterion_feature(student_output_dict['unet_out'], teacher_output_dict['unet_out'])
            else:
                l_feature = 0

            loss = l_dense + \
                    args.w_chamfer * l_chamfer + \
                    args.w_bound * l_boundary + \
                    args.w_center * l_center + \
                    args.w_feature * l_feature + \
                    args.w_prob * l_prob
            # loss = l_dense + args.w_chamfer * l_chamfer
            loss.backward()
            ...
            if should_log and step % 5 == 0:
                ...
                wandb.log({f"Train/Total_loss": loss.item()}, step=step)
            ...
            if should_write and (step % args.validate_every == 0 or i == len(train_loader) - 1):
                ...
                # print("Time elapsed to eval:", model.ed_time)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quant', type=str, help='set quantization type. If not set, quantization is disabled. supported: float16, TODO: qint8', default='')
    parser.add_argument('--teacher_model', type=str, help='checkpoint file path of teacher model', required=True)
    ...
    parser.add_argument('--w_bound', type=float, help='', default=0.2)
    parser.add_argument('--w_feature', type=float, help='', default=0.02)
    parser.add_argument('--w_center', type=float, help='', default=0.2)
    parser.add_argument('--w_prob', type=float, help='', default=0.2)
...
# --encoder_backend tf_efficientnet_b0