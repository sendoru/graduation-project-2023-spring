...
class InferenceHelper:
    def __init__(self, video_filename='', dataset='nyu', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            model = UnetAdaptiveBins.build(n_bins=256, min_val=self.min_depth, max_val=self.max_depth, basemodel_name='tf_efficientnet_b0').to(device)
    ...
    def run_video(self, filename):
        ...
        while True:
            ...
            if not ret:
                break
            # TODO transform to PIL needed (mayde)
            center, pred = self.predict_pil(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            pred = pred.squeeze()
            pred_min = np.min(pred)
            pred_max = np.max(pred)
            pred_rgb = (cmap((pred - self.min_depth) / (2 - self.min_depth)) * (2**16 - 1)).astype(np.uint16)[:,:,:3]
            cv2.imshow('frame', cv2.cvtColor(pred_rgb, cv2.COLOR_RGB2BGR))
            print(f"FPS: {1/(time() - time_start):.3f}")
            cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    inferHelper.run_video('output_L_near.mp4')
